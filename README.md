...
- **Its error findings include false positives: 57% on the first real measurement.** Of 7 errors
  on the first live run against a real site, **4 of 7 were the tool's fault**, all from pages
  reached through a redirect being counted as duplicates of the pages they redirect to. That defect is fixed, but the number is here
