...
updated_at: "2026-09-22T04:45:00-06:00"
record_status: open
work_status: active
---
...
| **U10** (granted; **held** — needs the kind-awareness answer) | **`json-ld` means the home page, and the verdict names the page it looked at.** Today this consumer accepts JSON-LD on **any crawled page**, so a site with it only on a deep page passes; the rule Heron has written makes it home-page-only | `siteplan/docs/PLAN-FORMAT.md`, revised by Heron. Resolves **G1** | Moss, **after the principal confirms Heron's answer to the kind-awareness caveat** | Three criteria: (1) `json-ld` is met only when the **home page** carries a `@type`; (2) the finding names the page it looked at, so a reader can tell which page was checked; (3) the tests assert the change on what a machine reads, including a site whose JSON-LD is only on a deep page, which **passes today and must not after** | 0.5 focus session. **Held pending Heron's ruling, with three options before it.** (a) home-page-only, which is what the draft rule says and which would fail a conforming content site, whose `Article` markup belongs on its articles; (b) kind-aware, which would make the surface check depend on the plan's `kind` — a structural change to what is currently a dictionary lookup, and it would need the document to say consumers may branch on `kind`; (c) **the principal's third option: `json-ld` as a surface means "the site publishes JSON-LD somewhere" and names the page it found it on, leaving the front-door requirement to `identity.schema_types`, which is already checked against the home page.** Under (c) U10 shrinks to naming the page, with no consumer branching on `kind` at all |
