...
Increment 1 (U1–U16) is recorded as written through revision 37. Revision 38 opens increment 2,
unit U17, which is unit U4 of the tools-as-skills record in the perspicuity repository, carried
here under that record's Grant U4; the card is copied beside U17 in Act.

## Current position

Principal and decider: David.

Mode: **`Run`**, for increment 2. U17 proceeds under Grant U4 of the tools-as-skills record, which
David ratified at 2026-09-23T02:32:38Z (20:32:38 -06:00 on 2026-09-22). It stopped at the offline
half's return, and resumes when the live half is released.

Work owner: Moss (coordinator), acting as the worker-U4 subagent for U17 only.

Claimed by: none. Moss (worker-U4 subagent) held U17 from "2026-09-22T22:23:04-06:00"
(2026-09-23T04:23:04Z) and released the claim at the offline half's return,
"2026-09-22T22:30:13-06:00". The live half takes a new claim when Primary dispatches it.

Decision: `inherited` for increment 2 — course D of the tools-as-skills record, selected by David
at 2026-09-23T02:27:40Z on that record's revision 2 (`7d60cdef` in the perspicuity repository).
U17 selects nothing among that decision's alternatives; it measures the number the course's
`sitewalk` skill may quote. Increment 1's decision is below, as written.

Work scope: increment 2, **U17 only** — re-measure this tool's false-positive rate on the current
build, recorded here. **This dispatch covers the offline half: the `--dir` run.** The live half is
held: a live run against `findmynextbite.food` would fall inside `agenttrace`'s untouched
observation week (its R8, due 2026-09-28), and David has not chosen its timing. No request goes to
any live host under this dispatch.

Done when: as Grant U4, beside U17 in Act — one live run and one `--dir` run, every error
inspected against the page, the rate recorded with method, sample, date and limits.

Ship to: this `RECORD.md` at a commit; Primary, the tools-as-skills orchestrator, reads it.

Work: revision 38 registered U17's claim, grant card, pickup plan and disposition rule at
`b44706e`, before any run. Revision 39 returns the offline half: delivered to Primary for reading.
The live half is blocked. U17 is not complete and is not submitted as complete.

Outcome: the offline half's evidence is in Act, "U17 offline half returned". It is dated
2026-09-23 and limited to two files of a server-rendered site. It is not a rate the skill may quote.

Next: Primary — read the offline half in Act, and accept it as the offline half or send it back.

Further pending: David, to choose when the live half runs (option A, on or after 2026-09-28, or B,
now), in the tools-as-skills record's "U4's targets". David, to grant or decline U18, the
relative-canonical defect the offline half found.

Blocked: the live half, for want of David's timing choice. The offline half is returned.

Waiting on: David, for the live half's timing.

Dependency: David chooses option A or B in the tools-as-skills record's "U4's targets".

Increment 1's position follows, kept as written at revision 37. Its Outcome, Next and Q2 lines
predate the real-target runs that Act and Review record for U4, U13 and U16.
...
Next: Moss — nothing is outstanding in this grant. **B1–B3 need another project's release path**
and cannot be observed from this repository; report the delivery and stop.
Further pending: David, to answer Q2 by naming a project whose built directory U4 may run against.

Dependency: Nothing blocks anything. B1–B3 wait on an adopting project, which is not a dependency
...
| --- | --- | --- |
| `sitewalk/` — 15 modules | **2,592 lines at registration (2026-09-21)**, the baseline U1 reconciled against. The current size is not restated here: it changes with every unit, and the record keeps this number because it is what the plan was made about | Reconciled and delivered in U1–U3; see the unit table for their state |
| `tests/` — 9 test modules plus `fakes.py` | **2,378 lines at registration (2026-09-21)**, the same baseline. Modules and lines since then are not restated; `tools/field_count.py` reports the field count on demand, and `make ci` reports the test count | Delivered |
...
| `python3 -m unittest discover -s tests -t .` | **238 of 240 at registration**; the two failures are diagnosed below and were resolved in U2. The current count is printed by `make ci` and is not restated here | Resolved |
...
| **U15** (delivered) | **A real plan, generated by `siteplan new`, checked against a real build by `sitewalk --plan`.** No `--plan` check has ever met a real plan — only fixtures and conformance cases — which is the end-to-end gap `siteplan` recorded as unestablished. Both halves exist and need no project | `siteplan new`; the `agent-eligibility` build from U4. Closes the gap **U4's plan amendment** recorded | Moss, after U13 and U14 | Three criteria: (1) a plan generated by the producer is read without a fault by this consumer; (2) the surface and identity checks report against the real build, and a surface the build does not publish is reported unmet or unverified as the rules require; (3) the run is recorded with its findings reviewed against the build's files, so the result is checked rather than believed | 0.25 focus session. Uncertainty **low** |
| **U16** (delivered) | **A redirecting page's content findings are misattributed to it, generalised from U13.** The duplicate checks excluded pointers; the other content checks did not, so run 4 named `/company/` in `no_json_ld` while the login page it redirects to was named in the same message | Five live runs against `findmynextbite.food`, `tests/test_dir_mode.py::ARedirectingPageIsAPointer`, and two injected wrong changes | Moss, 2026-09-22 | **Met, verified live and by breaking it both ways.** A page reached through a redirect is excluded from **every** content check — titles, descriptions, canonicals, JSON-LD — while staying visible as a redirect, and a target's genuine gap is reported once against the target. Live: run 5's `no_json_ld` names `/about/`, `/open-food-data/` and `/accounts/login/` and **no longer `/company/`**. **The error total is unchanged at 15 and that is not the measure**: the site's other findings dominate, and what changed is which pages are named. Both wrong changes fail 3 tests and `make ci`: excluding the pointer from the redirect report as well, and restoring the pre-U16 filter | Accepted. Both halves are asserted, as required — the pointer produces no content finding, **and** the redirect is still reported — because a test asserting only the absence would not notice a page quietly vanishing |
...
| **U17** (waiting: offline half returned at revision 39; live half blocked on David's timing) | **Re-measure the false-positive rate on the current build.** Increment 2; unit U4 of the tools-as-skills record. The 57% that U4 measured predates U13 and U16 | Grant U4, copied in "Increment 2" below; targets named by David at 2026-09-23T04:19:06Z: `findmynextbite.food` (live, held) and its build directory | Moss (worker-U4 subagent), since "2026-09-22T22:23:04-06:00"; claim released at the offline return | As Grant U4, with the acceptance criteria in the U17 pickup plan. Accepted by Primary. Offline half: 29 errors, 1 false positive, 27 partial-build; see "U17 offline half returned" | Grant tolerance: 8 hours and 3 failed attempts at any one check; 7 minutes used at the return |
| **U18** (planned, not granted) | **Under `--dir --origin`, a relative canonical is judged against the synthetic origin and gates as `canonical_other_host`.** Found by U17's run 1 and reproduced on a one-page scratch build | U14's `--origin` design; `sitewalk/pages.py` and `sitewalk/findings.py` | Moss, if David grants it | A build whose canonical is `href="/"` passes under `--origin`; a build naming another site's host still gates; the `canonical_other_path` note stops calling the page's own address a different path | 0.5 focus session, estimated |
...
| --- | --- | --- | --- |
| G1 | **`json-ld` was "Schema.org JSON-LD in the HTML of the pages it describes"** — which pages was never defined. The only vocabulary surface that is not a URL, so the only one with no fetch to define it | **Resolved by the format, in two parts.** (a) The first fix defined it as the home page; **that was superseded before any consumer built to it** — Heron adopted the principal's third option, on the ground that home-page-only would fail a `content-site` whose `Article` markup belongs on its articles, and that the fix for that would have been a consumer branching on `kind`, which is the defect shape this review found. The live rule: markup on **some page the site serves**, the front-door requirement carried by `identity.schema_types`, **no consumer branching on `kind`**, and the consumer **naming the page** it found the markup on. (b) The naming half was a real gap here and is delivered as **U12**. `siteplan/docs/PLAN-FORMAT.md` at `7e77be1` | A second implementer reading the frozen `fe8433b` would still be guessing; against `7e77be1` the rule is explicit and this consumer now implements all four clauses |
...
### The outage that interrupted U4's live re-run

Recorded because two runs of the same command gave different numbers and the reason was outside this
project.

`findmynextbite.food` was down from roughly 05:29Z to 06:38Z on 2026-09-22. The principal's account
of the cause: the service runs under `DynamicUser=yes`, a restart left its data directory owned by a
stale UID, and the process could not read its own database, so every view touching it returned 500
while `/healthz` stayed 200. A later restart re-chowned correctly.

**The timing in that account was inferred, not measured, and the principal has corrected it.**
05:18:16Z was the service restart; this project's run 1 served 50 pages at 05:28:42Z and completed at
05:29:12Z, so the site was healthy eleven minutes after the restart that was blamed, and broke
between 05:29Z and 05:33Z. The cause is real; the claim that the *restart* broke it was a
correlation nobody checked. This record keeps the measured times and does not reconcile the
boundary, which is the right treatment when one account is measured and the other inferred.

**The sitemap growth is part of the same deploy, not a separate change.** 40 URLs became 253, and the
host carries an `app.sqlite3.pre-catalogue-load-20260922` backup written at 05:18 — so the restart,
the outage and the catalogue load are one event. That also explains why run 3 at the original 50-page
budget never reached the pages under test: the budget was consumed by the sitemap itself.

**What this is an instance of.** A cause and an observed time were joined into a single claim without
checking that they coincided. It is the same shape as the stale evidence sentences and the
unreproducible counts elsewhere in this record, and it is the eighth instance of the class: the fix
is to state which parts are measured and which are inferred, rather than to state the conclusion the
two suggest together.
...
### U4 pickup plan amendments from the runs

Two findings from the runs are tool defects rather than anything about the targets, and each is
registered above rather than fixed inside U4 — a unit that fixes what it finds cannot say what it
found.

**Why `--dir`'s origin problem is the more serious of the two.** The offline gate is the product's
centre: it is what `CONTEXT.md` calls the deploy gate, and O1 rests on it. On a real build it emits
an error that gates on a page doing exactly the right thing, because the build declares its true
origin and the tool judges it against a synthetic one. The result is not a false positive on an edge
case — it is **every page of every build that sets a canonical**, which is every build that a
`siteplan` brief produced, since the brief requires one.

**What the runs did not show, stated so it is not assumed.** Neither run exercised a `--plan`
check against a real plan: `agent-eligibility` has no plan file, and no project here has produced
one. The `--plan` path has still only met fixtures and `siteplan`'s conformance cases.

### Increment 2: U17, the false-positive rate re-measured

This increment exists because another record asked for it. The tools-as-skills record in the
perspicuity repository (id `tools-as-skills-2026-09-22`, file
`docs/initiatives/tools-as-skills-2026-09-22/WORK.md`) gives this tool a `SKILL.md` under its
course D. Its section "How a 57% error rate is handled" rules that the skill may quote only a rate
measured on the current build. The 57% (4 of 7 live errors, at U4 on 2026-09-22) predates U13 and
U16, which fixed its cause. U17 measures the replacement. The decision and its alternatives live in
that record; this section holds the grant, the route and the evidence.

"Primary" is that record's own name for the orchestrator that dispatched this unit and accepts it.
This repository retires the word as a role label; it is kept because it is the only name that record
gives.

Local choices, made at pickup:

```
Chose: the local unit id U17 over reusing U4, because this record already has a delivered U4 (the run that measured the 57%) and a second U4 would make "U4's rate" ambiguous; commit trailers keep the batch's id (Perspicuity-Unit: U4)
Decided by: Moss, as the worker-U4 subagent, under Grant U4
Reconsider if: Primary or David wants the batch's unit ids used verbatim in tool records
```

```
Chose: to act and sign as Moss, the coordinator named in docs/ACTORS.md, over "worker-U4 subagent" alone, because this repository names every actor and the grant makes this worker its coordinator for the unit; the parent's label stays beside the name so the two records join
Decided by: Moss, as the worker-U4 subagent, under Grant U4
Reconsider if: Primary or David names this worker differently
```

```
Chose: to run --dir with --origin https://findmynextbite.food over running it with no origin, because since U14 that is how the gate is meant to meet a real build, and without it the canonical host check is silent, so the run with the origin is the stricter test of the tool (O3 of the tools-as-skills record)
Decided by: Moss, as the worker-U4 subagent, under Grant U4
Reconsider if: the skill U3 writes invokes --dir without --origin, in which case that configuration is the one to measure
```

```
Chose: to run the tool inside a network namespace with no interfaces (unshare -rn) over relying on --dir's own guarantee, because the guarantee is what a false claim would hide, and a kernel that refuses every connection turns "no request was sent" from an assertion into a property of the run
Decided by: Moss, as the worker-U4 subagent, under Grant U4
Reconsider if: unshare is unavailable where the run is repeated; the socket-refusing patch in tests/fakes.py is the fallback
```

```
Chose: the disposition rule below, registered before the run, over counting every finding that would not hold on the live site as a false positive, because the 57% counted only the tool's own faults and a rate that mixes in the sample's shape would not be comparable with it; the partial-build count is reported beside it so nothing is hidden
Decided by: Moss, as the worker-U4 subagent, under Grant U4
Reconsider if: Primary or David counts partial-build findings as false positives; the per-finding table supports that recount without a re-run
```

**Grant U4: re-measure `sitewalk`'s false-positive rate**

Copied from the tools-as-skills record at revision 4 (`16ab54ff`), with its "as the U4 row" and
"as U1" references written out; the targets are from revision 10 (`63110e52`).

- For: a worker subagent in the `sitewalk` repository.
- Serves: course D → O3 (an error finding is never reported as a fact). It bears on this record's
  O1, whose direction includes "false gates down".
- Intent: the rate the skill quotes describes the current build, so a superseded 57% is never
  presented as current.
- Done when: one live run and one `--dir` run against real targets; every error inspected against
  the page; the rate recorded with method, sample, date and its remaining limits in that
  repository's record.
- Ship to: `sitewalk`'s `RECORD.md` at a commit; the receiver reads it.
- Includes / Excludes: read-only runs against the targets David names, and the record entry / any
  change to the tool, any write to a target, and any fetch beyond the named targets.
- Tolerances: eight hours and three failed attempts at any one check.
- Escalate if: the problem, comparison or selection changes; a tolerance is exceeded; an excluded
  target is needed.
- Return to: the tools-as-skills record's Act section, with the method, the sample, the dated rate
  and its limits.
- Accepted by: Primary.
- Granted by: David, ratifying the batch's cards at 2026-09-23T02:32:38Z; basis the tools-as-skills
  record at revision 3. Targets named by David at 2026-09-23T04:19:06Z: `findmynextbite.food`
  (live) and its build directory. Primary released only the build-directory run; the live run is
  held for David's timing choice.

#### U17 pickup plan (registered before any run)

The build directory is `public/` in the `find-my-next-bite` repository. It holds two HTML pages,
`index.html` and `open/index.html`; every other page is rendered live by the site's Python server.
The offline sample is therefore two pages, and that is all the static build holds.

Route:

1. Fingerprint the sample, read-only: the target's `HEAD`, `git status` for `public/`, and the
   SHA-256 of both HTML files, before and after the run.
2. Run this repository's current build, at its committed `HEAD` with a clean tree, inside
   `unshare -rn`, so any socket the tool opened would fail:
   `python3 -m sitewalk --dir <target>/public --origin https://findmynextbite.food`, once with
   `--json` and once as text with `--strict` for the exit code. The report is deterministic, so the
   two are one measurement in two renderings. Output goes to `.u4/u17/`, which is gitignored local
   scratch as it was for U4.
3. Inspect every error and conditional finding against the file it names. Read the bytes with
   `grep` and a short script that uses the standard library's parser and never imports `sitewalk`.
   Where a finding turns on a path the build does not hold, read the site's server source to see
   whether it renders that path, and mark that part inferred from source rather than measured.
4. Record the method, sample, date, each disposition, the counts and the limits in this section,
   and mark U17 as the offline half returned with the live half held.
5. Run `make ci` and `make records`.

How the route serves the intent: the skill may quote only a measured rate. The offline half
measures the deploy gate, which is the path the skill invokes and the one a release depends on. The
live half, where the 57% came from, stays unmeasured. Recording the offline figure as partial and
dated keeps a half-measured number from being quoted as the rate.

**The disposition rule**, registered before the run. Each error and conditional finding gets one of:

| Disposition | Meaning |
| --- | --- |
| confirmed | What the finding states is true of the named file or files, read by hand |
| false positive | What it states is false of the files: the tool misread the bytes or judged them against the wrong standard. This is the tool's fault, the class the 57% counted |
| not checked | Not verified, with the reason |

A confirmed finding that holds only because the build is two pages of a server-rendered site is
also marked **partial-build**. It is not the tool's fault and it is not hidden: it measures the
gate's value on this site, not the tool's accuracy. The offline false-positive rate is false
positives over error and conditional findings; the partial-build count is stated over the same
denominator. The tool's evidence discipline applies: a short visible-text length is not evidence
of an empty page, and a page that looks script-rendered is reported as looking script-rendered.

Acceptance criteria for the offline half:

1. The run completes: exit 0 by default, and 0 or 1 by its findings under `--strict`, never 2.
2. The run is offline by construction: the namespace refuses every connection.
3. Every error and conditional finding carries a disposition and names what was inspected.
4. The method, sample, date, offline false-positive count and limits are recorded here.
5. U17 is marked offline half returned, live half held. It is not marked submitted as complete.
6. Nothing is written in `find-my-next-bite`: the SHA-256s and `git status` of `public/` match
   before and after.
7. `make ci` and `make records` exit 0.

U17 is reviewed by Primary against the tools-as-skills record's R3, which is registered there and
not restated here.

#### Route adaptation, 2026-09-22T22:27:58-06:00, after run 1 and before run 2

Run 1 read only `index.html`: the tool finds pages by following links from `/` and by the build's
sitemap, the build has no sitemap, and nothing in `index.html` links `/open/`. The dispatch's sample
is two pages, so the second page needs its own run.

```
Chose: a second read-only --dir run rooted at public/open, over recording open/index.html as unread, because it is the only file in the build that the server sends as-is (views.open_layer_page), so it is the one page on which the tool's reading can be tested against bytes a visitor receives; its link findings are marked as artifacts of the re-rooting, since the page's real address is /open-food-data/ and every other route lives in the server
Decided by: Moss, as the worker-U4 subagent, under Grant U4 (the Includes permit read-only runs against the named build directory)
Reconsider if: Primary rules that only a run rooted at the build directory counts toward the offline half; run 2 is then reported and not counted
```

#### U17 offline half returned, 2026-09-22T22:30:13-06:00; live half held

**The offline half is returned to Primary, which reads it and accepts it or sends it back. U17 is
not complete and is not submitted as complete.** The live half has not run. No request went to any
live host.

**Finding.** On this sample the offline gate produced 29 error findings and no conditional ones.
**One is a false positive, and it is a new tool defect that is not specific to this site**: under
`--dir --origin`, a relative canonical is resolved against the synthetic `https://localhost` walk
origin and then reported as a gating `canonical_other_host`. Of the other 28, 27 are true of the
files but exist only because the build is two files of a server-rendered site. One is true of a page
the server sends as-is. **These counts are not a false-positive rate the skill may quote**; see the
limits below.

**Method.** This repository at `b44706e` (package unchanged since `9725892`, U16), `sitewalk 0.1.0`,
clean tree. Each run went through the real entry point inside `unshare -rn`, a network namespace
whose only interface is a downed loopback, so any socket would have failed the run. Output is in
`.u4/u17/` (gitignored, local).

| Run | Command (target `T=/home/david/projects/find-my-next-bite`) | At | Exit | Findings |
| --- | --- | --- | --- | --- |
| 1 | `unshare -rn python3 -m sitewalk --dir $T/public --origin https://findmynextbite.food` with `--json`, and with `--strict` | 2026-09-23T04:25:16Z | 0 by default, 1 under `--strict` | 18 error, 0 conditional, 5 note; 16 URLs, 19 requests to the directory source |
| 2 | the same with `--dir $T/public/open` | 2026-09-23T04:28:15Z | 0 by default, 1 under `--strict` | 11 error, 0 conditional, 6 note; 10 URLs, 14 requests |

**Sample.** `public/` holds two HTML files and nothing else is a page; every other address is
rendered by the server. The two files are not alike, and this is read from the server's source, not
measured:

- `index.html` is a **Django template source**. `backend/core/site.py` reads it with
  `engines["django"].from_string(...)` and renders it per request, so its raw bytes are never sent.
  It carries `{% if %}`, `{% include %}` and `{{ }}` placeholders, including the canonical and the
  JSON-LD.
- `open/index.html` is a generated static page. `backend/core/views.py::open_layer_page` sends it
  verbatim at `/open-food-data/`, and `/open/` redirects there. It is the only file in the build
  that a visitor receives as-is.

Run 1 read only `index.html`. The tool finds pages by links from `/` and by the build's sitemap. The
build has no sitemap and `index.html` does not link `/open/`, so run 2 put the second page through
the tool.

The target changed while U17 ran. Another actor made five commits in `find-my-next-bite` between
04:22Z and 04:28Z (`d109c4b` to `373a174`). So each sample is pinned by SHA-256 to a commit:

| File | SHA-256 (first 16) | Identical to | Run |
| --- | --- | --- | --- |
| `public/index.html` | `dca68372e50c3e16` | `912ec49:public/index.html` | 1 |
| `public/open/index.html` | `bab23e36d64b5a9f` | `373a174:public/open/index.html`, unchanged across all five commits | 2 |

Run 1 reproduces from the commit. The tree was extracted with `git archive 912ec49 public` into a
scratch directory and re-run. The JSON report is identical apart from times and the source path.

**Dispositions.** Each finding was checked against the bytes with a script that uses
`html.parser` and `json` and never imports `sitewalk`, and with `grep`. Server routes were read in
`backend/config/urls.py`: inferred from source, not fetched.

| Run | Finding | Disposition | What was inspected |
| --- | --- | --- | --- |
| 1 | `canonical_other_host`: `/ -> https://localhost/{{ canonical }}` | **false positive** | Line 9: `<link rel="canonical" href="{{ canonical }}">`, a relative reference. Resolved against the page's address on the declared origin, its host is `findmynextbite.food`. `sitewalk/pages.py` resolves it against `page.url` (`https://localhost/`), and `findings.py` compares that host with the declared one. Reproduced on a one-page scratch build with `href="/"`: it gates. The same page with an absolute canonical passes. The canonical is broken, an unrendered placeholder, but "on another host" is false. A correct tool would report at most `canonical_other_path`, which is a note |
| 1 | `malformed_json_ld`: `/` | confirmed; partial-build (template) | Line 19: the block's content is `{{ website_jsonld }}`, and `json.loads` fails. The server substitutes generated JSON when it renders the page (`site.py`) |
| 1 | `no_json_ld`: `/` | confirmed; partial-build (template) | The only JSON-LD block is that placeholder, so the file carries no `@type` |
| 1 | `page_not_ok`: 14 URLs | confirmed; partial-build | No file under `public/` for `/why/`, `/foods/?q=seitan+in+Hamilton`, `/foods/?q=smoked+tofu+for+sandwiches`, `/foods/type/lentils/`, `/foods/`, `/what-we-know/`, `/accounts/logout/`, `/foods/{{ food.pk }}/`, `/api/discovery`, `/llms.txt`, `/books/`, `/{{ destination.href }}`, `/about/`, `/company/`. Each has a server route. Two are unrendered template links |
| 1 | `broken_internal_link` ×14, one per path above | confirmed; partial-build | Each is an `<a href>` in `index.html`, lines 41–70, and each target file is absent. These are the same 14 facts as `page_not_ok` |
| 2 | `no_json_ld`: `/` (the open page) | **confirmed**, not partial-build | No `application/ld+json` block in the file, by `grep` and by the parser. The server sends this file verbatim, so the served page carries none (inferred from source, not fetched) |
| 2 | `page_not_ok`: 9 URLs | confirmed; partial-build | No file for `/foods/`, `/knowledge/`, `/accounts/login/`, `/shops/`, `/open-food-data/`, `/what-we-know/`, `/books/`, `/why/`, `/about/`, whether rooted at `public/open` or `public/`: every link is root-absolute, so re-rooting changed none of them. Each has a server route |
| 2 | `broken_internal_link` ×9, one per path above | confirmed; partial-build | Each `<a href>` sits on lines 15–17 or 872. These are the same 9 facts as `page_not_ok` |

| Count (error and conditional) | Run 1 | Run 2 | Both |
| --- | --- | --- | --- |
| Findings | 18 | 11 | 29 |
| False positive (the tool's fault) | 1 | 0 | 1 |
| Confirmed, partial-build | 17 | 10 | 27 |
| Confirmed, true of a served page | 0 | 1 | 1 |
| Not checked | 0 | 0 | 0 |

The evidence discipline held. No page was flagged as script-rendered. The pages read carry 6,373
and 77,837 characters of visible text. The 23 missing paths are reported as unknown rather than
absent, and no page is called empty.

Two observations below error severity are not counted. The absolute-canonical reproduction draws a
`canonical_other_path` note saying a canonical at the page's own address on the declared origin is
"a different path", which has the same cause. In run 2, `canonical_missing` is true.

**Limits.**

1. **Offline half only.** The live path is where the 57% was measured, and it is unmeasured. The
   rate is not final.
2. **Two files, and one is not a built page.** All 18 of run 1's findings come from a template the
   server never sends. The only page tested against
   bytes a visitor receives is `open/index.html`.
3. **Granularity inflates the denominator.** `page_not_ok` and `broken_internal_link` report the
   same 23 missing paths twice. Counted as distinct facts, run 1 has 17 facts with 1 false and run 2
   has 10 with none. Any ratio from this sample depends on that choice, so it is not comparable with
   4 of 7 on 50 live pages.
4. **Server routes are inferred from source.** Some sit behind flags, such as `/why/` and
   `/company/`. Whether each answers 200 in production was not observed.
5. **Self-check.** Moss read the runs. Primary's reading is the receiving check. No independent
   assessor has looked.
6. **A possible write to the target's Git metadata.** The first five Git status reads used plain
   `git status`, which may refresh `.git/index`'s stat cache. It changes no tracked content. Every
   later Git read in the target set `GIT_OPTIONAL_LOCKS=0`. Nothing else was written:
   `open/index.html`'s hash is unchanged. `index.html` changed only through the other actor's
   commit `373a174`.

**What this means for the gate, as evidence and not a decision.** On this directory `--strict` exits
1 on every run. Of the 29 gating findings, 27 are true but say nothing about whether a release broke
anything, one is true of a served page, and one is false. A gate on this directory would fail every
release of this site, which is the switched-off gate the tools-as-skills record warns about. The 27
come from the sample's shape and the one false finding from the tool. Whether `--strict` may gate is
Q3 of that record, and it is David's.

**Acceptance criteria for the offline half.**

| # | Criterion | Result |
| --- | --- | --- |
| 1 | Completes; 0 by default, 0 or 1 under `--strict`, never 2 | Met: both runs exit 0, and 1 under `--strict` |
| 2 | Offline by construction | Met: `unshare -rn`, where a connection attempt returns "Network is unreachable" |
| 3 | Every error and conditional finding has a disposition and names what was inspected | Met: 29 of 29 in the table |
| 4 | Method, sample, date, offline false-positive count and limits recorded | Met: this section |
| 5 | Marked offline half returned, live half held; not submitted as complete | Met: the unit row and Current position say so |
| 6 | Nothing written in `find-my-next-bite` | Met for tracked content, with limit 6 on `.git/index` |
| 7 | `make ci` and `make records` exit 0 | Met at the return commit; the tails are in the return to Primary |

**Tolerance.** Seven minutes of the eight hours had elapsed at the return, and no check failed. The
eight hours run from pickup, 2026-09-23T04:23:04Z. A live half under option A, on or after
2026-09-28, falls outside them, so it needs David's amendment of the tolerance or a fresh grant
clock. This is recorded for Primary and is not a request to change the grant.

**Proposals, not part of the return.** Each needs a grant this unit does not hold:

- **U18, planned and not granted**: resolve page addresses against `--origin` under `--dir`, so a
  relative canonical is judged on the declared host. Tests would cover a build with `href="/"`,
  which must pass, and a build naming another site's host, which must still gate. The
  `canonical_other_path` wording has the same cause. The defect bears on the rate the skill will
  quote, so it may belong before the live half. Owner if granted: Moss. Grant: David.
- **`--dir` names the built HTML files it did not reach.** Here it read one of two and said
  nothing. A deploy gate that silently skips a built page is the same blind spot that "sitemap URLs
  never reached" already covers for the live path.
- **A static build for the offline half.** This directory cannot measure the tool's accuracy on
  more than one served page. `agent-eligibility`'s build, which U4 and U14 used, is one candidate.
  It is not a named target, so only David can name it.
- **Record upkeep outside U17.** The 0.6.0 checker warns that increment 1's Review table has no
  Serves column. Increment 1's Outcome, Next and Q2 lines predate its own runs. Both belong in a
  record-only commit by Moss outside this grant.
...
| U15 — a real plan from the producer, checked against a real build | The plan `siteplan new` wrote and `siteplan check` validated, the plan block and findings from the run, and `tests/test_plan.py::ARealPlanFromTheProducer` | Moss, 2026-09-22 | **Met — the gap is closed.** `siteplan new --kind saas` produced a plan with 5 surfaces, identity types, 8 pages; `siteplan check` passed it; `sitewalk --plan` read it **with no fault and no ignored keys**, which is the reading claim. The verdict is `not met`, with two findings **both verified by hand against the build**: no `rss.xml` exists, and the home page carries `Organization/Service/Offer` but not the plan's `WebSite`. So the producer's generator and this consumer agree end to end on a real plan for a real build, and the two findings are true | Accepted. `json-ld` was satisfied through the derived surface while `identity.schema_types` failed on the home page — **the split from U10/U12 doing exactly what it was designed to do, on a real plan** |
| U13 — a redirecting page is not its target | **Four live runs against `findmynextbite.food`**, with their timestamps; `tests/test_dir_mode.py::ARedirectingPageIsNotItsTarget`; and the site's own sitemap fetched independently | Moss, 2026-09-22 | **Met, live before-and-after obtained.** Run 1 (before the fix, 05:28:42Z, 50 pages) produced **4 errors naming the two redirecting pages**; run 4 (after the fix, 15:07:32Z, 300 pages) produced **0 for `/accounts/logout/` and 1 for `/company/`**, and the redirects are reported as redirects. The one that remains is **true but misattributed** — see U16. **The runs are not a clean comparison and the record says so:** the site's sitemap grew from **40 to 253 URLs** during the outage, so run 3 at the original 50-page budget never reached the redirect pages at all (0 of 50 pages redirected), and run 4 needed a 300-page budget to reach them by the tool's own advice. The budget itself changed between the runs | Accepted as a live before-and-after with the site-change caveat stated, and with the local reproduction as the controlled case |
...
target files read without the tool, and the server's source. Reason: a material result is recorded
with its method, sample, dispositions and limits, and a defect the review found is registered with
an owner rather than left in prose. What changed: Act gains the route adaptation (run 2, made before
it ran) and "U17 offline half returned"; U17's row moves to `waiting` and the claim is released; U18
is registered as planned and not granted; Review gains U17's row; Current position names Primary
next and David for the live half's timing and U18; the header's `work_status` moves to `waiting`.
What is preserved: revision 38's pickup plan and disposition rule, unchanged, and every increment 1
entry. Affects: U17, and U18 if David grants it.

Revision 38, 2026-09-22T22:23:04-06:00. **Increment 2 opens with U17, claimed under Grant U4 of the
tools-as-skills record.** Source: Primary's dispatch of the worker-U4 subagent, on David's
ratification of the grant (2026-09-23T02:32:38Z) and his naming of the targets
(2026-09-23T04:19:06Z). Reason: a unit's claim, grant and pickup plan are registered before it runs,
and the disposition rule before the findings it judges. What changed: the header moves to skill
0.6.0 as the compatibility rule asks; Current position carries increment 2 first, with a claim and
`since`, and keeps increment 1's lines as written with a note that three of them predate the
real-target runs; U17 joins the unit table; Act gains "Increment 2" with the grant card, five
short-form choices, the pickup plan, the disposition rule and seven acceptance criteria. The live
half is recorded as held. What is preserved: every increment 1 revision, its units and its review,
unchanged. Affects: U17 only.

Revision 37, 2026-09-22T10:45:00-06:00. **U16 is delivered, and the outage is recorded with the
measured/inferred split that explains it.** Source: U16's live verification, and the principal's
correction of the outage account. Reason: each planned result is marked delivered with its evidence,
and an account that joined a cause to an inferred time belongs in the record corrected rather than
reconciled. What changed: U16's row carries the live result and both failure modes; the outage
section records the cause, the **corrected** timing, that this project's runs are the measured ones,
and that the sitemap growth is the same deploy rather than a coincidence; and the class gains its
eighth instance. What is preserved: every earlier revision and B1–B3 as unobserved. Affects: nothing
outstanding in this grant.

Revision 36, 2026-09-22T10:15:00-06:00. **U13's live before-and-after is obtained, and the residual
it exposes is registered as U16.** Source: four live runs against `findmynextbite.food` on
2026-09-22, after the principal confirmed the target was healthy again and explained the outage
window. Reason: the acceptance check that could not be met was met. What changed: U13's row carries
the live comparison with all four runs' timestamps; the site-change caveat is recorded, because the
sitemap grew 40 → 253 URLs between the runs and the page budget had to change with it; and U16
registers the residual the deeper run exposed — a content finding attributed to a redirecting page
that does not have content of its own. What is preserved: every earlier revision, and B1–B3 as
unobserved. Affects: U16, which needs a ruling on its shape before it is implemented.
...
delivered with its evidence. What changed: U15's row carries the run and its two hand-checked
findings; `tests/test_plan.py::ARealPlanFromTheProducer` records the run as a test that **skips**
when its inputs are absent rather than passing silently. What is preserved: every earlier revision.
Affects: nothing outstanding except the benefit claims B1–B3, which need another project's release
path. **The plan-and-build inputs live in `.u4/`, which is gitignored** — the test skips without
them, and that is recorded rather than hidden.
...
runs showed, including the four false errors on the live site and the one on the build, and the two
defects are U13 and U14; the honest boundary is restated — two targets, one day, read by the hand
that wrote the tool. What is preserved: every earlier revision. Affects: U13 and U14 are now the
outstanding units; **U4 is no longer blocked and no longer open**.

Revision 30, 2026-09-22T06:45:00-06:00. **U4's two runs are done, and they found two tool defects,
registered as U13 and U14.** Source: the runs themselves, on the principal's authorisation. Reason:
the runs exposed behaviour no fixture could — the redirect artifact and the invented origin — and a
unit that fixes what it finds cannot report what it found. What changed: U13 and U14 are registered
with their criteria; the pickup plan records why the origin defect is the more serious of the two,
and what the runs did **not** show, which is any `--plan` check against a real plan. What is
preserved: every earlier revision. Affects: U13 and U14 after U4 returns.
...
`siteplan/docs/PLAN-FORMAT.md` at `7e77be1`, read rather than taken on report. Reason: the unit
dissolved because the format moved to what this consumer already did — but **two of the three
clauses were already true and the third was not**: the live rule requires a consumer checking
`json-ld` to name the page it found the markup on, and this consumer named none, reporting only a
count. Recording the withdrawal without checking the claim would have released a hold over an unmet
requirement. What changed: U10 is marked withdrawn with its reason and the revision; U12 is
registered and delivered with its evidence; G1 is resolved and its history kept, including that the
home-page rule was superseded by a named replacement rather than deleted; and the derived surface
carries the pages it was derived from. What is preserved: every earlier revision and every other
G-finding. Affects: **U4 is once again the only outstanding unit**, blocked on Q2.

Revision 27, 2026-09-22T04:45:00-06:00. **The pre-plan account stops restating counts, and U10's
hold names all three options.** Source: a structural sweep of this record for the staleness class,
and the principal's third option relayed on 2026-09-22. Reason: the rule adopted for `Current
position` — a summary carries pointers, not counts — applies to every section that restates a
derived fact, and the pre-plan table still carried "2,658 after", "2,894 after" and "277 pass after",
all of which were true when written and are false now. What changed: that table keeps only the
**registration baseline**, which is the number the plan was actually made about, and points at
`make ci` and `tools/field_count.py` for current values; U10's row records the three options before
Heron including the principal's, so the hold does not read as a question only this consumer raised.
What is preserved: every earlier revision. Affects: the record's own currency, and U10 when Heron
rules.
