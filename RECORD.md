...
updated_at: "2026-09-22T00:20:00-06:00"
record_status: open
work_status: active
---
...
| U9 — the deletion test, run across the package | The attribute-level scan (result below), `tests/test_guard.py::TheAddressHoldsOnlyWhatIsRead`, `::NoDeadErrorTypes`, `tests/test_plan.py::ASubCheckReportsWhetherItRan`, and two injected wrong changes | Moss, at U9 delivery, 2026-09-21 | **Met, and the re-run returned a finding rather than nil.** `Address.host/port/family` and `PlanError` are removed. The scan ran **before** the fixes and examined **115 fields**, reporting **one** populated-but-never-read field, `PageSource.label` — a false positive, since it *is* read through `getattr(source, "label", "")`, which an AST scan cannot see; the real defect was the Protocol declaration promising a member no source must implement, now corrected to declare only `fetch` and `exists`. **After the fixes the same scan examines 113 fields and returns zero**, which is the verifiable end state: the count fell by two because `Address` lost three fields and gained none, and the field-count arithmetic is stated so an assessor re-running the scan does not read a nil result as a contradiction of the finding. The plan's `required_surfaces_met: true` beside `passed: false` is fixed: a sub-check that did not run reports `None`, not `True`. Both wrong changes fail tests and `make ci` | Accepted as a self-check. The one hit being a declaration error rather than dead code is the honest result, and it is recorded as such rather than as a clean nil |
...
| U7 — the verdict is honest about what it knows | `tests/test_plan.py::TheVerdictIsHonestAboutWhatItKnows`, `::AnUncheckedSurfaceIsUnverifiedNotAbsent`, and the injected original defect | Moss, at U7 delivery, 2026-09-21 | **Met, and verified against the true defect.** Four version cases: 1 and older read clean; newer or unknown is `met with conditions` and gates; absent or mistyped is `not met` and gates, with a message that says *malformed* rather than *may be newer*. A vocabulary surface this tool cannot check is **unverified**, never absent — `rss.xml` produces `met with conditions` and a `conditional` finding that says the tool has no check for it. The third severity is defined in the output and reaches `finding_counts` and `severities` in the JSON. **Verification note: the first wrong change I injected did not change behaviour** — `rss.xml` has two independent routes to unverified — so I restored the *actual* original line (`surface is None or not surface.exists`) instead; that fails three tests | Accepted as a self-check, with the failed first mutation recorded because it is P1's rule catching me rather than a fixture |
