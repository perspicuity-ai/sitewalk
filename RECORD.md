...
updated_at: "2026-09-22T10:15:00-06:00"
record_status: open
work_status: active
---
...
Next: David — rule on U16's shape: exclude a redirecting page from every content check, as it now is
from duplicates, or keep it and mark it as serving its target's content. **B1–B3 still need another
project's release path.**
...
| **U16** (registered, not granted) | **A redirecting page's *content* findings are misattributed to it.** The duplicate checks exclude redirecting pages (U13), but the other content checks do not: run 4 reports `/company/` in `no_json_ld` although `/company/` redirects to `/accounts/login/`, which the same message also names. The finding is **true** — neither page carries markup — and the page it is attributed to is not the page at fault, which is the same misattribution U13 removed from duplicates | Run 4 of the live evidence, `.u4/live4.json`; the site's own response for `/company/` fetched independently | Moss, after the principal rules on the shape | **The decision, not the implementation:** a redirecting page should either be excluded from every content check, as it now is from duplicates — cleanest, since its facts belong to its target — or kept and marked as serving its target's content, so a reader knows where the finding really belongs. **The recommendation is exclusion from all content checks**, on the ground that U13's reasoning was not specific to duplicates: a page reached through a redirect does not have content of its own, so any finding about its content is a finding about its target, and the target is already reported. What must survive either way: the redirect itself stays visible, and a target's genuine gap is reported **once**, against the target | 0.25 focus session. Uncertainty **low** on the fix, medium on the choice |
